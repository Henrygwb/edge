...
# 4. (Construct K_{z,z} with a specific structure) Place inducting point (Z) on a grid. Here, the inducing points are
# fixed after defining the grid boundary and the grid size.
# 5. (Faster way of computing K_{x,z} in p(f|u)) KISS-GP (Structured kernel interpolation) :
#    Use the local kernel interpolation technique (Usually using this when inducing points are placed on grid),
#    to approximate K_{x,z}K_{z,z}^{-1} with the interpolation matrix M. When being used together with the SoR, we have
#     K_{xz} = MK_{zz}, and K_{xx} \appro MK_{zz}K_{zz}-1K_{zz}M^T = M K_{zz} M^T (This is the KSI kernel).
